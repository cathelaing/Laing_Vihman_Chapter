...
## top infants
basile_01 <- global_matrix_iter[["Basile_1"]]
write.table(basile_01,file="basile_01.txt")
adeline_01 <- global_matrix_iter[["Adeline_1"]]
write.table(adeline_01,file="adeline_01.txt")
vincent_01 <- global_matrix_iter[["Vincent_1"]]
write.table(vincent_01,file="vincent_01.txt")
eelis_01 <- global_matrix_iter[["Eelis_1"]]
write.table(eelis_01,file="eelis_01.txt")
mira_01 <- global_matrix_iter[["Mira_1"]]
write.table(mira_01,file="mira_01.txt")
afan_01 <- global_matrix_iter[["Afan_1"]]
write.table(afan_01,file="afan_01.txt")

## bottom infants
gwyn_01 <- global_matrix_iter[["Gwyn_1"]]
write.table(gwyn_01,file="gwyn_01.txt")
ella_01 <- global_matrix_iter[["Ella_1"]]
write.table(ella_01,file="ella_01.txt")
tobias_01 <- global_matrix_iter[["Tobias_1"]]
write.table(tobias_01,file="tobias_01.txt")
radia_01 <- global_matrix_iter[["Radia_1"]]
write.table(radia_01,file="radia_01.txt")
carys_01 <- global_matrix_iter[["Carys_1"]]
write.table(carys_01,file="carys_01.txt")
...
distance_melted_iter <- reshape2::melt(global_matrix_iter) %>%   # turn list into a df
  rename("gloss1" = "Var1",
         "gloss2" = "Var2",
         "distance" = "value") %>%
...
globaldistance_iter <- as.data.frame(distance_melted_iter)

globaldistance_list_iter <- globaldistance_iter %>%
  split(., f = .$Speaker_ID)
  

globaldistance_list_iter_updated <- lapply(globaldistance_list_iter, 
...
globaldistance_iter_final <- reshape2::melt(globaldistance_list_iter_updated) %>%
  ...
  dplyr::select(-maxdist)  %>%
  mutate(data_type = "iter")

write_csv(globaldistance_iter_final, "globaldistance_iter.csv")


##### 2. Thresholds - probably don't need this

globaldistance_iter <- read_csv("globaldistance_iter.csv")
...
globaldistance_iter_list <- lapply(thresholds, function(t) {
  filter(globaldistance_iter, distance_norm < t) %>%
    group_by(Speaker_ID, gloss1, data_type) %>%                     # for gloss1 side of the data, otherwise 50% of data is missed (oops)
    tally()
})

globaldistance_iter_list_melted <- reshape2::melt(globaldistance_iter_list)

globaldistance_iter_list_degree <- globaldistance_iter_list_melted %>%
  rename("degree" = "value") %>%
  separate(L1, into = c("remove", "threshold"), sep = "_") %>%
  filter(threshold == 0.25) %>%
  dplyr::select(-remove, -threshold, -variable)

globalthresholds_iter <- globaldistance_iter_list_melted %>%
  rename("degree" = "value") %>%
  separate(L1, c("l1", "threshold"), sep = "_") %>%
  dplyr::select(-l1, -variable)

write_csv(globalthresholds_iter, "globalthresholds_iter.csv")
...
thresholds_iter <- read_csv("globalthresholds_iter.csv") %>%
...
infants_subj_I <- thresholds_iter %>% dplyr::select(Speaker_ID, gloss1)

infants_list_subj_I <- infants_subj_I %>%
  split(., f = .$Speaker_ID)

graph_base_subj_I <- globaldistance_iter %>%              # create edges
  distinct(Speaker_ID, gloss1, distance, .keep_all = TRUE) %>%
  rename("from" = "gloss1",
         "to" = "gloss2",
         "weight" = "distance_norm")

globalnodes_subj_I <- graph_base_subj_I %>%      # create nodes
  select(-to) %>% 
...
globalgraphdata_subj_I <- lapply(infants_list_subj_I, FUN = function(element) {
  edges_net <- graph_base_subj_I %>% filter(Speaker_ID %in% element$Speaker_ID) %>% distinct(word_pair, distance, .keep_all = TRUE)
  nodes_net <- globalnodes_subj_I %>% filter(Speaker_ID %in% element$Speaker_ID)
...
globalclusteringcoef_global_base_subj_I <- lapply(globalgraphdata_subj_I, FUN = function(element) {
  global_clustering_coef <- transitivity(element, type = "average")
})

globalclusteringcoef_global_subj_I <- reshape2::melt(globalclusteringcoef_global_base_subj_I) %>%
  rename("clust_coef_global" = "value", 
         "Speaker_ID" = "L1") 

globalsmallworlddata_subj_I <- globalpathlength_subj_I %>% 
  left_join(globalclusteringcoef_global_subj_I, by = c("Speaker_ID")) %>%
  separate(Speaker_ID, c("Speaker", "ID"), sep = "_", remove = F) %>%
  left_join(subj_lang) %>%
  mutate(data_type = "iter")

write_csv(globalsmallworlddata_subj_I, "globalsmallworlddata_iter.csv")
