...
for (i in 1:num_samples) {
  samples_list_T[[i]] <- data_list_iter_base_T %>% group_by(Subject) %>% slice_sample(n=20)
}

# view the first sample
#samples_list[[1]]
samples_unlist_T <- do.call(rbind.data.frame, samples_list_T)
...
data_list_iter_T <- samples_unlist_T %>% 
  ungroup() %>%
  mutate(listID = listID) %>%
  mutate(subj_ID = paste(Subject, listID, sep="_")) %>%
  split(., f = .$subj_ID)

global_matrix_iter_T <- lapply(data_list_iter_T, FUN = function(element) {
...
## top infants
basile_01_T <- global_matrix_iter_T[["Basile_1"]]
write.table(basile_01_T,file="basile_01_T.txt")
adeline_01_T <- global_matrix_iter_T[["Adeline_1_T"]]
write.table(adeline_01_T,file="adeline_01_T.txt")
vincent_01_T <- global_matrix_iter_T[["Vincent_1_T"]]
write.table(vincent_01_T,file="vincent_01_T.txt")
eelis_01_T <- global_matrix_iter_T[["Eelis_1_T"]]
write.table(eelis_01_T,file="eelis_01_T.txt")
mira_01_T <- global_matrix_iter_T[["Mira_1_T"]]
write.table(mira_01_T,file="mira_01_T.txt")
afan_01_T <- global_matrix_iter_T[["Afan_1_T"]]
write.table(afan_01_T,file="afan_01_T.txt")

## bottom infants
gwyn_01_T <- global_matrix_iter_T[["Gwyn_1_T"]]
write.table(gwyn_01_T,file="gwyn_01_T.txt")
ella_01_T <- global_matrix_iter_T[["Ella_1_T"]]
write.table(ella_01_T,file="ella_01_T.txt")
tobias_01_T <- global_matrix_iter_T[["Tobias_1_T"]]
write.table(tobias_01_T,file="tobias_01_T.txt")
radia_01_T <- global_matrix_iter_T[["Radia_1_T"]]
write.table(radia_01_T,file="radia_01_T.txt")
carys_01_T <- global_matrix_iter_T[["Carys_1_T"]]
write.table(carys_01_T,file="carys_01_T.txt")
...
distance_melted_iter_T <- reshape2::melt(global_matrix_iter_T) %>%   # turn list into a df
  rename("gloss1" = "Var1",
         "gloss2" = "Var2",
         "distance" = "value") %>%
...
globaldistance_iter_T <- as.data.frame(distance_melted_iter_T)

globaldistance_list_iter_T <- globaldistance_iter_T %>%
  split(., f = .$Speaker_ID)


globaldistance_list_iter_updated_T <- lapply(globaldistance_list_iter_T, 
                                             FUN = function(element) {
                                               ...
                                               actual_globaldistance <- actual_globaldistance_speaker %>%
                                                 ...
                                                 distinct(gloss1, Speaker_ID, distance, .keep_all = TRUE)
                                               actual_globaldistance_final <- list(actual_globaldistance)
                                             })
#check <- reshape2::melt(globaldistance_list_iter_updated_T)
...
data_list_iter_A <- samples_unlist %>% 
  ungroup() %>%
  mutate(listID = listID) %>%
  mutate(subj_ID = paste(Subject, listID, sep="_")) %>%
  split(., f = .$subj_ID)

global_matrix_iter_A <- lapply(data_list_iter_A, FUN = function(element) {
...
distance_melted_iter_A <- reshape2::melt(global_matrix_iter_A) %>%   # turn list into a df
  rename("gloss1" = "Var1",
         "gloss2" = "Var2",
         "distance" = "value") %>%
  filter(gloss1 != gloss2) %>%
...
globaldistance_iter_A <- as.data.frame(distance_melted_iter_A)

globaldistance_list_iter_A <- globaldistance_iter_A %>%
  split(., f = .$Speaker_ID)


globaldistance_list_iter_updated_A <- lapply(globaldistance_list_iter_A, 
                                           FUN = function(element) {
...
globaldistance_iter_A_final <- reshape2::melt(globaldistance_list_iter_updated_A) %>%
  dplyr::select(-L2, -variable) %>%
  rename("distance" = value) %>%
  ungroup() %>%
  separate(L1, c("Speaker", "listID"), sep = "_")

globaldistance_iter_T_final <- reshape2::melt(globaldistance_list_iter_updated_T) %>%
  dplyr::select(-L2, -variable) %>%
  rename("distance" = value) %>%
  ungroup() %>%
  separate(L1, c("Speaker", "listID"), sep = "_")

max_dist <- rbind(globaldistance_iter_A_final, globaldistance_iter_T_final) %>%
  summarize(highest_dist = max(distance))

globaldistance_iter_T_final <- globaldistance_iter_T_final %>%
  mutate(distance_norm = distance/(max_dist$highest_dist)) %>%
  mutate(data_type = "iter_T") %>% 
  write_csv("globaldistance_iter_T.csv")

globaldistance_iter_A_final <- globaldistance_iter_A_final %>%
  mutate(distance_norm = distance/(max_dist$highest_dist)) %>%
  mutate(data_type = "iter_A") %>% 
  write_csv("globaldistance_iter_A.csv")


##### 2. Thresholds - probably don't need this

### Target

globaldistance_iter_T <- read_csv("globaldistance_iter_T.csv")
...
globaldistance_iter_T_list <- lapply(thresholds, function(t) {
  filter(globaldistance_iter_T, distance_norm < t) %>%
    group_by(Speaker_ID, gloss1, data_type) %>%                     # for gloss1 side of the data, otherwise 50% of data is missed (oops)
    tally()
})

globaldistance_iter_list_melted_T <- reshape2::melt(globaldistance_iter_T_list)

globaldistance_iter_list_degree_T <- globaldistance_iter_list_melted_T %>%
  rename("degree" = "value") %>%
  separate(L1, into = c("remove", "threshold"), sep = "_") %>%
  filter(threshold == 0.25) %>%
  dplyr::select(-remove, -threshold, -variable)

globalthresholds_iter_T <- globaldistance_iter_list_melted_T %>%
  rename("degree" = "value") %>%
  separate(L1, c("l1", "threshold"), sep = "_") %>%
  dplyr::select(-l1, -variable)

write_csv(globalthresholds_iter_T, "globalthresholds_iter_T.csv")

### Actual

globaldistance_iter_A <- read_csv("globaldistance_iter_A.csv")
...
globaldistance_iter_A_list <- lapply(thresholds, function(t) {
  filter(globaldistance_iter_A, distance_norm < t) %>%
    group_by(Speaker_ID, gloss1, data_type) %>%                     # for gloss1 side of the data, otherwise 50% of data is missed (oops)
    tally()
})

globaldistance_iter_list_melted_A <- reshape2::melt(globaldistance_iter_A_list)

globaldistance_iter_list_degree_A <- globaldistance_iter_list_melted_A %>%
  rename("degree" = "value") %>%
  separate(L1, into = c("remove", "threshold"), sep = "_") %>%
  filter(threshold == 0.25) %>%
  dplyr::select(-remove, -threshold, -variable)

globalthresholds_iter_A <- globaldistance_iter_list_melted_A %>%
  rename("degree" = "value") %>%
  separate(L1, c("l1", "threshold"), sep = "_") %>%
  dplyr::select(-l1, -variable)

write_csv(globalthresholds_iter_A, "globalthresholds_iter_A.csv")
...
thresholds_iter_T <- read_csv("globalthresholds_iter_T.csv") %>%
  separate(Speaker_ID, c("Subject", "ID"), sep = "_", remove = F) #%>% 
...
infants_subj_IT <- thresholds_iter_T %>% dplyr::select(Speaker_ID, gloss1)

infants_list_subj_IT <- infants_subj_IT %>%
  split(., f = .$Speaker_ID)

graph_base_subj_IT <- globaldistance_iter_T %>%              # create edges
  distinct(Speaker_ID, gloss1, distance, .keep_all = TRUE) %>%
  rename("from" = "gloss1",
         "to" = "gloss2",
         "weight" = "distance_norm")

globalnodes_subj_IT <- graph_base_subj_IT %>%      # create nodes
  select(-to) %>% 
...
globalgraphdata_subj_IT <- lapply(infants_list_subj_IT, FUN = function(element) {
  edges_net <- graph_base_subj_IT %>% filter(Speaker_ID %in% element$Speaker_ID) %>% 
    distinct(word_pair, distance, .keep_all = TRUE)
  nodes_net <- globalnodes_subj_IT %>% filter(Speaker_ID %in% element$Speaker_ID)
  net_plot <- graph_from_data_frame(d=edges_net, directed=F, vertices=nodes_net) 
...
globalpathlength_base_subj_IT <- lapply(globalgraphdata_subj_IT, FUN = function(element) {
  path_length <- mean_distance(element, directed = F)
  mean_k <- mean(degree(element))
  output <- merge(path_length, mean_k)
})

globalpathlength_subj_IT <- reshape2::melt(globalpathlength_base_subj_IT) %>%
...
globalclusteringcoef_global_base_subj_IT <- lapply(globalgraphdata_subj_IT, FUN = function(element) {
  global_clustering_coef <- transitivity(element, type = "global")
})

globalclusteringcoef_global_subj_IT <- reshape2::melt(globalclusteringcoef_global_base_subj_IT) %>%
  rename("clust_coef_global" = "value", 
         "Speaker_ID" = "L1") 

globalsmallworlddata_subj_IT <- globalpathlength_subj_IT %>% 
  left_join(globalclusteringcoef_global_subj_IT, by = c("Speaker_ID")) %>%
  separate(Speaker_ID, c("Speaker", "ID"), sep = "_", remove = F) %>%
  left_join(subj_lang) %>%
  mutate(data_type = "iter_T")

# globalsmallworlddata_subj_I %>% group_by(Speaker, Language) %>% 
#   summarise(meanPL = mean(path_length),
#             meank = mean(mean_k))
...
distance_df <- read_csv("distance_df.csv") %>% dplyr::select(
  Subject, Language, Age, Gloss, GlossID, geminate_A, geminate_T, data_type) %>%
  rename(gloss1 = GlossID)
thresholds_iter_A <- read_csv("globalthresholds_iter_A.csv") %>%
  separate(Speaker_ID, c("Subject", "ID"), sep = "_", remove = F) #%>% 
#left_join(distance_df)
subj_lang <- distance_df %>% group_by(Subject, Language) %>% tally() %>%
  rename("Speaker" = Subject) %>%
...
infants_subj_IA <- thresholds_iter_A %>% dplyr::select(Speaker_ID, gloss1)

infants_list_subj_IA <- infants_subj_IA %>%
  split(., f = .$Speaker_ID)

graph_base_subj_IA <- globaldistance_iter_A %>%              # create edges
  distinct(Speaker_ID, gloss1, distance, .keep_all = TRUE) %>%
  rename("from" = "gloss1",
         "to" = "gloss2",
         "weight" = "distance_norm")

globalnodes_subj_IA <- graph_base_subj_IA %>%      # create nodes
  select(-to) %>% 
  distinct(from, Speaker_ID, .keep_all = TRUE)
...
globalgraphdata_subj_IA <- lapply(infants_list_subj_IA, FUN = function(element) {
  edges_net <- graph_base_subj_IA %>% filter(Speaker_ID %in% element$Speaker_ID) %>% 
    distinct(word_pair, distance, .keep_all = TRUE)
  nodes_net <- globalnodes_subj_IA %>% filter(Speaker_ID %in% element$Speaker_ID)
  #checks <- c(edges_net, nodes_net)
  net_plot <- graph_from_data_frame(d=edges_net, directed=F, vertices=nodes_net) 
  net_plot_threshold <- delete_edges(net_plot, which(E(net_plot)$weight > cut.off))    # delete edges with a threshold above .25
})
...
globalpathlength_base_subj_IA <- lapply(globalgraphdata_subj_IA, FUN = function(element) {
  path_length <- mean_distance(element, directed = F)
  mean_k <- mean(degree(element))
  output <- merge(path_length, mean_k)
})

globalpathlength_subj_IA <- reshape2::melt(globalpathlength_base_subj_IA) %>%
  pivot_wider(names_from = variable, values_from = value) %>%
  rename("path_length" = "x",
         "mean_k" = "y",
         "Speaker_ID" = "L1")
...
globalclusteringcoef_global_base_subj_IA <- lapply(globalgraphdata_subj_IA, FUN = function(element) {
  global_clustering_coef <- transitivity(element, type = "global")
})

globalclusteringcoef_global_subj_IA <- reshape2::melt(globalclusteringcoef_global_base_subj_IA) %>%
  rename("clust_coef_global" = "value", 
         "Speaker_ID" = "L1") 

globalsmallworlddata_subj_IA <- globalpathlength_subj_IA %>% 
  left_join(globalclusteringcoef_global_subj_IA, by = c("Speaker_ID")) %>%
  separate(Speaker_ID, c("Speaker", "ID"), sep = "_", remove = F) %>%
  left_join(subj_lang) %>%
  mutate(data_type = "iter_A")

globalsmallworlddata_iter <- rbind(globalsmallworlddata_subj_IA, globalsmallworlddata_subj_IT)

write_csv(globalsmallworlddata_iter, "globalsmallworlddata_iter.csv")
