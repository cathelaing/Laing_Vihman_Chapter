...
data_list_target <- distance_df %>%   
  filter(data_type == "target") %>%
  group_by(Subject, Gloss) %>%
  slice(1) %>% 
  split(., f = .$Subject)

global_matrix_target <- lapply(data_list_target, FUN = function(element) {
...
distance_melted_target <- reshape2::melt(global_matrix_target) %>%   # turn list into a df
  rename("gloss1" = "Var1",
         "gloss2" = "Var2",
         "distance" = "value") %>%
...
globaldistance_target <- as.data.frame(distance_melted_target)

globaldistance_list_target <- list(globaldistance_target)

globaldistance_list_target_updated <- lapply(globaldistance_list_target, 
  ...
  actual_globaldistance <- actual_globaldistance_speaker %>%
    ...
    distinct(gloss1, Speaker, .keep_all = TRUE) 
  actual_globaldistance_final <- list(actual_globaldistance)
})

globaldistance_target_final <- reshape2::melt(globaldistance_list_target_updated) %>%
...
data_list_actual_base <- distance_df %>%   
  ungroup() %>%
...
for (i in 1:num_samples) {
  samples_list[[i]] <- data_list_actual_base %>% group_by(Subject) %>% slice_sample(n=20)
}
...
data_list_actual <- samples_unlist %>% 
  ungroup() %>%
  mutate(listID = listID) %>%
  mutate(subj_ID = paste(Subject, listID, sep="_")) %>%
  split(., f = .$subj_ID)

global_matrix_actual <- lapply(data_list_actual, FUN = function(element) {
...
distance_melted_actual <- reshape2::melt(global_matrix_actual) %>%   # turn list into a df
  rename("gloss1" = "Var1",
         "gloss2" = "Var2",
         "distance" = "value") %>%
  filter(gloss1 != gloss2) %>%
...
data_list_actual_lang <- distance_df %>%   
  filter(data_type == "actual") %>%
  split(., f = .$Subject)

global_matrix_actual_lang <- lapply(data_list_actual_lang, FUN = function(element) {
...
distance_melted_actual_lang <- reshape2::melt(global_matrix_actual_lang) %>%   # turn list into a df
  rename("gloss1" = "Var1",
         "gloss2" = "Var2",
         "distance" = "value") %>%
  filter(gloss1 != gloss2) %>%
...
globaldistance_actual_lang <- as.data.frame(distance_melted_actual_lang)

globaldistance_list_actual_lang <- list(globaldistance_actual_lang)

globaldistance_list_actual_updated_lang <- lapply(globaldistance_list_actual_lang, 
                                             FUN = function(element) {
...
globaldistance_actual_final_lang <- reshape2::melt(globaldistance_list_actual_updated_lang) %>%
  dplyr::select(-L1, -L2, -variable) %>%
  ungroup() %>%
  rename("distance" = value) %>%
  #group_by(Speaker) %>%
  mutate(maxdist = max(distance),
         distance_norm = distance/maxdist) %>%
  dplyr::select(-maxdist)  %>%
  mutate(data_type = "actual_lang",
         Speaker_ID = Speaker,
         listID = NA)

globaldistance_final <- rbind(globaldistance_target_final, globaldistance_actual_final,
                              globaldistance_actual_final_lang)
