...
data_ex <- distance_df %>%   
  filter(data_type == "actual" & Subject == "Ivy") %>%
  #group_by(Gloss) %>%
  #slice(1) %>%
  slice_sample(n=5)

data_ex$GlossID <- gsub("[[:digit:][:punct:]]", "", data_ex$GlossID)

data_list_ex <- data_ex %>%   
  split(., f = .$Subject)

global_matrix_ex <- lapply(data_list_ex, FUN = function(element) {
